...
public class CRM_RegisterActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		loadingBar = CustomProgressAlert.make(this, "Please wait...");
		...
		button.setOnClickListener(view -> {
			...
			if (s_name.isEmpty()) {
				...
				customer.setDomain(s_domain);
				customer.setSub_domain(s_subdomain);
				customer.setPackage_value(s_packagevalue);
				...
				Call<JsonObject> call = ri.insertCustomer(customer);
				call.enqueue(new Callback<JsonObject>() {
					@Override
					public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
						if (response.isSuccessful()) {
							CustomToast.makeText(CRM_RegisterActivity.this, "Customer added" + response.message(), 0, Color.GREEN);
						} else {
							CustomToast.makeText(CRM_RegisterActivity.this, "Failed" + response.message(), 0, Color.RED);
						}
					}

					@Override
					public void onFailure(Call<JsonObject> call, Throwable t) {
						CustomToast.makeText(CRM_RegisterActivity.this, "Failed" + t.getMessage(), 0, Color.RED);
					}
				});
